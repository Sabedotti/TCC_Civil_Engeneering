...
def desenho_secao_transversal_pilar(dim_x, dim_y, nx, ny, d_linha, bitola_aco):
    ...
    canvas = np.ones((largura_fundo_x, largura_fundo_y, 3)) * 255 #imagem 400x300, com fundo branco e 3 canais para as cores
    cv2.waitKey(0)
    maior_face = dim_x
    if dim_y > dim_x:
        maior_face = dim_y
    ...
    p1x, p1y = int(largura_fundo_x/2 - dim_x / 2 * multiplicador), int(largura_fundo_y/2 + dim_y / 2 * multiplicador)
    p2x, p2y = int(p1x + dim_x * multiplicador), int(p1y)
    p3x, p3y = int(p1x), int(p1y - dim_y * multiplicador)
    p4x, p4y = int(p1x + dim_x * multiplicador), int(p1y - dim_y * multiplicador)
    ...
    for num_armadura, coord_x , coord_y in posicao_barras:
        ...
        cv2.circle(canvas, (int_coord_x, int_coord_y), (int(bitola_aco / 20 * multiplicador)), vermelho, -1)
