...
def definicao_aco_area_compressao(posicao_barras, dim_xy, passo, bitola_aco):
    ...
    while i < dim_xy:
        ...
        if i < lista[indice_1][indice_2]:
            ...
            # area_de_aco.append(0)
            lista_areas = determinacao_area_bitolas_compressao_concreto(
                bitola_aco, passo)
...
def areas_de_concreto(posicao_xy_barras, comp_secao, altura, bitola_aco):
    passo = 0.01
    areas_aco_secao = definicao_aco_area_compressao(
        posicao_xy_barras, comp_secao, passo, bitola_aco)
    i = passo
    indice = 0
...
def calc_compressao_concreto(bx, deformacao_maxima_para_bx, ec2, fcd, n, cg,
                             comp_secao, areas_concreto):
    passo = 0.01
    i = passo
...
def forcas_concreto(passo, bx, def_max_concreto, comp_secao, ec2, fcd, coef_n,
                    area_concreto_por_passo, cg, fck):
    ln = bx
    fcd_cm = fcd / 10
    ...
    while i < i_max:
        ...
        tensao_2023 = tensao_concreto_2023(
            fck, deformacao_da_area, ec2, fcd_cm, coef_n)
    ...
    return somat_forcas_normais, somat_momentos, somat_forcas_normais_2023, \
        somat_momentos_2023
...
def forcas_subtracao_areas_aco(pontos_centrais_aco, bitola_aco, bx, cg,
                               def_max_concreto, ec2, fcd, coef_n,
                               lista_areas_aco_para_subtracao, fck):
    ...
    for item in pontos_centrais_aco:
        ...
        if inicio_da_bitola >= ln:
            ...
            if ln < fim_interacao:
                fim_interacao = ln
            numero_iteracoes = round(
                (fim_interacao - inicio_da_bitola) / passo, 0)
            ...
            while i < numero_iteracoes:
                ...
                tensao_2023 = tensao_concreto_2023(
                    fck, media_defs, ec2, fcd_cm, coef_n)
                forca_normal = tensao * areas_para_subtracao[i] * (-1)
                forca_normal_2023 = tensao_2023 * \
                    areas_para_subtracao[i] * (-1)
    ...
    return f_normal_total, momento_total, f_normal_total_2023, \
        momento_total_2023


def forca_concreto_total(pontos_centrais_aco, bitola_aco, bx,
                         def_max_concreto, comp_secao, ec2, fcd, coef_n,
                         area_concreto_por_passo, cg,
                         lista_areas_aco_para_subtracao, fck):
    concreto_cheio_fn, concreto_cheio_momento, concreto_cheio_fn_2023, \
        concreto_cheio_momento_2023 = forcas_concreto(0.01, bx,
                                                      def_max_concreto,
                                                      comp_secao, ec2, fcd,
                                                      coef_n,
                                                      area_concreto_por_passo,
                                                      cg, fck)
    subtracao_areas_fn, subtracao_areas_momento, subtracao_areas_fn_2023, \
        subtracao_areas_momento_2023 = forcas_subtracao_areas_aco(
            pontos_centrais_aco, bitola_aco, bx, cg, def_max_concreto, ec2,
            fcd, coef_n, lista_areas_aco_para_subtracao, fck)
    ...
    forca_momento_total_2023 = concreto_cheio_momento_2023 + \
        subtracao_areas_momento_2023

    return (forca_normal_total, forca_momento_total, forca_normal_total_2023,
            forca_momento_total_2023)


# limite entre tração máxima e limite 1
def limite_tracao_max(posicao_primeira_barra, posicao_ultima_barra, fyd,
                      mod_elasticidade, es):
    limite = posicao_ultima_barra + \
        ((posicao_ultima_barra - posicao_primeira_barra)
         * es / ((fyd / mod_elasticidade) - es))
    return limite
...
def limite_3(ecu, fyd, modulo_elasticidade, posicao_ultima_barra):
...
def limite_4a(dimensao, d_linha, posicao_ultima_barra):
...
def calc_limite_tracaomax_1(bx_secao, posicao_xy_barras, dim_xy, es, fyd,
                            bitola, mod_elasticidade):
    bx = bx_secao
    bx_calculos = bx * (-1)
    ...
    for barra in pos_barras:
        ...
        deslocamento_barra = (barra + bx_calculos) * es / \
            (bx_calculos + ultima_linha_barras)
    ...
    return [sum(forcas_normais), sum(forcas_momento)], \
        [sum(forcas_normais), sum(forcas_momento)]


def calc_limite_1_2(bx_indicado, posicao_xy_barras, bitola_aco, ec2, es,
                    mod_elasticidade, fyd, dim_xy, fcd, coef_n, comp_secao,
                    area_concreto_por_passo, lista_areas_aco_para_subtracao,
                    fck
                    ):
    bx = bx_indicado
    ...
    forcas_normais_2023 = []
    forcas_momento_2023 = []
    forca_normal_concreto, forca_momento_concreto, \
        forca_normal_concreto_2023, forca_momento_concreto_2023 = \
        forca_concreto_total(posicao_xy_barras, bitola_aco, bx,
                             def_max_concreto, comp_secao, ec2, fcd, coef_n,
                             area_concreto_por_passo, cg,
                             lista_areas_aco_para_subtracao, fck)
    ...
    return [sum(forcas_normais) * (-1), sum(forcas_momento)/100], \
        [sum(forcas_normais_2023) * (-1), sum(forcas_momento_2023)/100]


def calc_limite_2_3(bx_indicado, bitola_aco, posicao_xy_barras,
                    mod_elasticidade, fyd, fcd, dim_xy, ecu, ec2, coef_n,
                    comp_secao, area_concreto_por_passo,
                    lista_areas_aco_para_subtracao, fck):
    bx = bx_indicado
    bitola_cm = bitola_aco / 10  # cm
    pos_barras = posicao_xy_barras
...
def calc_limite_3_4(bx_indicado, bitola_aco, posicao_xy_barras,
                    mod_elasticidade, fyd, fcd, dim_xy, ecu, ec2, coef_n,
                    comp_secao, area_concreto_por_passo,
                    lista_areas_aco_para_subtracao, fck):
    bx = bx_indicado
    ...
    forca_normal_concreto, forca_momento_concreto, \
        forca_normal_concreto_2023, forca_momento_concreto_2023 = \
        forca_concreto_total(posicao_xy_barras, bitola_aco, bx,
                             def_max_concreto, comp_secao, ec2, fcd,
                             coef_n, area_concreto_por_passo, cg,
                             lista_areas_aco_para_subtracao, fck)
    forcas_normais.append(forca_normal_concreto)
    forcas_momento.append(forca_momento_concreto)
    forcas_normais_2023.append(forca_normal_concreto_2023)
...
def calc_limite_4_4a(bx_indicado, bitola_aco, posicao_xy_barras,
                     mod_elasticidade, fyd, fcd, dim_xy, ecu, ec2, coef_n,
                     comp_secao, area_concreto_por_passo,
                     lista_areas_aco_para_subtracao, fck):
    bx = bx_indicado
...
def calc_limite_4a_5(bx_indicado, bitola_aco, posicao_xy_barras,
                     mod_elasticidade, fyd, fcd, dim_xy, ecu, ec2,
                     coef_n, comp_secao, area_concreto_por_passo,
                     lista_areas_aco_para_subtracao, fck):
    bx = bx_indicado
    bitola_cm = bitola_aco / 10  # cm
    ...
    forca_normal_concreto, forca_momento_concreto, \
        forca_normal_concreto_2023, forca_momento_concreto_2023 = \
        forca_concreto_total(posicao_xy_barras, bitola_aco, bx,
                             ecu_calculos_compressao, comp_secao, ec2,
                             fcd, coef_n, area_concreto_por_passo, cg,
                             lista_areas_aco_para_subtracao, fck)
    forcas_normais.append(forca_normal_concreto)
    forcas_momento.append(forca_momento_concreto)
    forcas_normais_2023.append(forca_normal_concreto_2023)
...
def compressao_max(ec2, dim_x, dim_y, bitola_aco, quant_bitolas, fcd, coef_n,
                   mod_elasticidade, fyd, fck):
    bitola = bitola_aco / 10  # cm
    fcd_cm = fcd / 10  # kn/cm²
    fyd_cm = fyd / 10  # kn/cm²
    modulo_elasticidade = mod_elasticidade / 10  # kn/cm²
    ...
    tensao_compressao_concreto_2023 = tensao_concreto_2023(
        fck, ec2, ec2, fcd_cm, coef_n)

    forca_normal_concreto = tensao_compressao_concreto * area_concreto
    forca_normal_concreto_2023 = \
        tensao_compressao_concreto_2023 * area_concreto
...
def pontos_diagrama(sentido, hx, hy, posicao_barras_x, posicao_barras_y,
                    area_passo_concreto_x, area_passo_concreto_y, es, fyd,
                    bitola_aco, mod_elasticidade, ec2, fcd, coef_n,
                    areas_bitola_passo, fck, ecu, numero_bitolas,
                    posicao_primeira_barra_x, posicao_ultima_barra_x, d_linha,
                    posicao_primeira_barra_y, posicao_ultima_barra_y):
    limites = limites_estados_deformacao(sentido, posicao_primeira_barra_x,
                                         posicao_ultima_barra_x, fyd,
                                         mod_elasticidade, es, ec2, ecu,
                                         d_linha, hx, hy,
                                         posicao_primeira_barra_y,
                                         posicao_ultima_barra_y)
    ...
    larg_secao = hy
    area_concreto_p_passo = area_passo_concreto_x
    ...
    if sentido == 'y':
        ...
        larg_secao = hx
        area_concreto_p_passo = area_passo_concreto_y

    for item in lista_bx:
        if limites[0] <= item <= limites[1]:
            ponto, ponto_2023 = calc_limite_tracaomax_1(item, pos_barras,
                                                        comp_secao, es, fyd,
                                                        bitola_aco,
                                                        mod_elasticidade)
            ...
            ponto, ponto_2023 = calc_limite_1_2(item, pos_barras, bitola_aco,
                                                ec2, es, mod_elasticidade, fyd,
                                                comp_secao, fcd, coef_n,
                                                comp_secao,
                                                area_concreto_p_passo,
                                                areas_bitola_passo, fck)
            ponto_inverso = [ponto[0], (-1)*ponto[1]]
            ponto_inverso_2023 = [ponto_2023[0], (-1)*ponto_2023[1]]

        elif limites[2] < item <= limites[3]:
            ponto, ponto_2023 = calc_limite_2_3(item, bitola_aco, pos_barras,
                                                mod_elasticidade, fyd, fcd,
                                                comp_secao, ecu, ec2, coef_n,
                                                comp_secao,
                                                area_concreto_p_passo,
                                                areas_bitola_passo, fck)
            ponto_inverso = [ponto[0], (-1)*ponto[1]]
            ...
            ponto, ponto_2023 = calc_limite_3_4(item, bitola_aco, pos_barras,
                                                mod_elasticidade, fyd, fcd,
                                                comp_secao, ecu, ec2, coef_n,
                                                comp_secao,
                                                area_concreto_p_passo,
            ...
            ponto, ponto_2023 = calc_limite_4_4a(item, bitola_aco, pos_barras,
                                                 mod_elasticidade, fyd, fcd,
                                                 comp_secao, ecu, ec2, coef_n,
                                                 comp_secao,
                                                 area_concreto_p_passo,
            ...
            ponto, ponto_2023 = calc_limite_4a_5(item, bitola_aco, pos_barras,
                                                 mod_elasticidade, fyd, fcd,
                                                 comp_secao, ecu, ec2, coef_n,
                                                 comp_secao,
                                                 area_concreto_p_passo,
    ...
    ponto, ponto_2023 = compressao_max(ec2, comp_secao, larg_secao,
                                       bitola_aco, numero_bitolas, fcd,
                                       coef_n, mod_elasticidade, fyd, fck)
    lista_pontos_diagrama.append(ponto)
    lista_pontos_diagrama_2023.append(ponto_2023)
...
def limites_estados_deformacao(sentido, posicao_primeira_barra_x,
                               posicao_ultima_barra_x, fyd, mod_elasticidade,
                               es, ec2, ecu, d_linha, hx, hy,
                               posicao_primeira_barra_y,
                               posicao_ultima_barra_y
                               ):
    if sentido == 'x':
        limite_tracao_max_pilar = limite_tracao_max(
            posicao_primeira_barra_x, posicao_ultima_barra_x, fyd,
            mod_elasticidade, es)
        limite_1_pilar = limite_1()
        limite_2_pilar = limite_2(ecu, es, posicao_ultima_barra_x)
        limite_3_pilar = limite_3(
            ecu, fyd, mod_elasticidade, posicao_ultima_barra_x)
        ...
        limite_tracao_max_pilar = limite_tracao_max(
            posicao_primeira_barra_y, posicao_ultima_barra_y, fyd,
            mod_elasticidade, es)
        limite_1_pilar = limite_1()
        limite_2_pilar = limite_2(ecu, es, posicao_ultima_barra_y)
        limite_3_pilar = limite_3(
            ecu, fyd, mod_elasticidade, posicao_ultima_barra_y)
        limite_4_pilar = limite_4(ecu, posicao_ultima_barra_y)
        limite_4a_pilar = round(
            (limite_4a(hy, d_linha, posicao_ultima_barra_y)), 2)
